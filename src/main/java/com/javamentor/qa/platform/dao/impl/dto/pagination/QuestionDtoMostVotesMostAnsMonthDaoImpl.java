...
@Repository(value = "QuestionsMostVotesMostAnswersForMonth")
public class QuestionDtoMostVotesMostAnsMonthDaoImpl implements PageDtoDao<QuestionViewDto> {
    ...
    public List<QuestionViewDto> getItems(Map<Object, Object> param) {
        ...
        User userAuth = (User) param.get("userAuth");
        ...
        return entityManager.createQuery(
                        "SELECT new com.javamentor.qa.platform.models.dto.question.QuestionViewDto" +
                                ...
                                "q.user.imageLink, " +
                                "q.description, " +
                                "(SELECT COUNT(view.user) FROM QuestionViewed view WHERE view.question.id = q.id) as count_views, " +
                                ...
                                "(Select count(down.vote) from VoteQuestion down where down.vote = 'DOWN_VOTE' and down.question.id = q.id)) as count_valuable, " +
                                "SUM(r.count)/(COUNT(r.id) / nullif(COUNT(DISTINCT r.id),0)), " +   // было с стало nullif(COUNT(DISTINCT r.id),0))
                                "q.persistDateTime, " +
                                ...
                                "AND q.persistDateTime >=:month " +
                                "AND q.isDeleted = false " +
                                "GROUP BY q.id, q.user.fullName,q.user.imageLink " +
                                "ORDER BY count_valuable desc, count_answers desc, count_views desc", QuestionViewDto.class)
                .setParameter("trackedIds", trackedIds)
                .setParameter("ignoredIds", ignoredIds)
                .setParameter("userId", userAuth.getId())
                .setParameter("month", localDateTime.minusMonths(1L))
